-- L'admin di prodotto può elencare i registrati di ogni venue
-- (prima staff_list_memberships richiedeva venue_staff).
...
begin
  ...
  from lobby.profiles p
  where p.id = auth.uid();

  if not coalesce(v_is_admin, false)
     and not lobby_private.is_venue_staff(p_venue_id, auth.uid()) then
